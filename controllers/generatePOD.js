...
exports.generatePOD = async (req, res) => {
  try {
    ...
    doc.fontSize(20).fillColor("#003366").text("Moova Logistics.", 150, 50, { align: "right" });
    doc.fontSize(10).fillColor("#555555").text("13 Powerel Street, Germiston | +27 11 225 2679 |  info@moova.co.za", { align: "right" });
    doc.moveDown(2);
    ...
    doc.fontSize(10).fillColor("#555555").text(
      `Generated on: ${new Date().toDateString()} | Moova | +27 11 225 2679 |  info@moova.co.za`,
      50,
      footerY + 5,
      { align: "center" }
    );
...
exports.generatePODAdmin = async (req, res) => {
  try {
    ...
    doc.fontSize(12).fillColor("#000000");
    doc.text(`Driver: ${load.driver?.name || load.driver?.email || "N/A"}`, 60, doc.y);
    doc.text(`Truck: ${load.truck?.registrationNumber || "N/A"}`, 300, doc.y - 15);
    doc.text(`Pickup: ${load.pickupLocation || "-"}`, 60, doc.y + 20);
    doc.text(`Delivery: ${load.deliveryLocation || "-"}`, 300, doc.y - 15);
    doc.moveDown(3);

    doc.text("Signatures:", { underline: true }).moveDown(0.5);
    doc.text(`Driver: ${load.driver?.email || "N/A"}`, 60, doc.y, { continued: true })
       .text(`Receiver: ${load.customer?.name || "N/A"}`, 300, doc.y, { align: "right" });
    doc.moveDown(3);
